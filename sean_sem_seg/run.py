...
if __name__ == "__main__":
    ...
    # parse all the point clouds in the directory provided from the command line
    parser = argparse.ArgumentParser()
    parser.add_argument("--dir", help="directory to parse")
    args = parser.parse_args()
    # get las files in the directory
    point_clouds_to_process = glob.glob(args.dir + "/**/*.las", recursive=True)

    # doing for the following point cloud paths
    print("Processing the following point clouds:")
    ...
    for point_cloud_filename in point_clouds_to_process:
        parameters = dict(
            ...
            batch_size=10,  # You will get CUDA errors if this is too high, as you will run out of VRAM. This won't be an issue if running on CPU only. Must be >= 2.
